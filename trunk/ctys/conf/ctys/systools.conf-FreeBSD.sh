...
if [ -z "$KSU" -a -z "$SUDO" -a "$USER" != "root" ];then
	...
	printWNG 2 $LINENO $BASH_SOURCE 1 "Probably limited access, missing:ksu and sudo, and $USER!=root"
    fi
else
...
[ -z "$CTYS_LSPCI" ]&&CTYS_LSPCI=`getPathName $LINENO $BASH_SOURCE WARNING lspci /usr/local/bin`
