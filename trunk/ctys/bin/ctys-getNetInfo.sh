...
#
#CALLFULLNAME:
CALLFULLNAME="Generic display of available Network Interfaces"
#
#LICENCE:
LICENCE=GPL3
#
...
while [ -n "$1" ];do
	...
	--iflistnames=*|--iln=*)
	       CMD=ILISTNAME;
               ILISTSCOPE=${1#*=};
               ILISTSCOPE=${ILISTSCOPE%%,*};
               ILISTSTRG=${1##*,}
	       [ "${ILISTSTRG}" == "$1" ]&&ILISTSTRG=;
               ;;

	--iflistnames|--iln)CMD=ILISTNAME;;

	--iflistmacip|--ilmi)CMD=IMACIP;;
	--iffirst|--if)CMD=IFIRST;;
...
function getNetInfo () {
    ILISTSCOPE=${ILISTSCOPE:-ALL};
    ILISTSTRG=${ILISTSTRG:-eth};

    printDBG $S_LIB ${D_BULK} $LINENO $BASH_SOURCE "$FUNCNAME:CMD=<${CMD}>"
    ret=0;
    case $CMD in
	ILISTNAME)netGetProcEthList ${ILISTSCOPE} ${ILISTSTRG};;
	IMACIP)netGetIFlst;;
	IFIRST)netGetFirstIf;;
	BLIST)netListBridges ${BLISTSTRG};;
	BLXEN)netListBridges ${BLISTSTRG:-peth};;
	BCXEN)[ -n "$BSTRG" ]&&netCheckBridgeIsXen ${BSTRG};ret=$?;;
	*)
	    _ARGS="${_ARGS} ${1}"
	    printDBG $S_LIB ${D_BULK} $LINENO $BASH_SOURCE "$FUNCNAME:CMD=${CMD}"
	    ;;
