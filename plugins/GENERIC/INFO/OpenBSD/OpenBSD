...
_myPKGNAME_OpenBSD_INFO="${BASH_SOURCE}"
_myPKGVERS_OpenBSD_INFO="01.02.001b01"
hookInfoAdd $_myPKGNAME_OpenBSD_INFO $_myPKGVERS_OpenBSD_INFO
...
function INFOOpenBSD () {
    ...
    if [ $? -eq 0 ]; then
  	printf "%"$((3*_allign))"s%-"${_label}"s:%"$((2*_allign))"s\n" \
          "  " "gawk" "`awk -V|sed -n '/awk version [0-9]*/p'`"
    else
  	printf "%"$((3*_allign))"s%-"${_label}"s:%"$((2*_allign))"s\n" \
