...
#
#FULLNAME:
FULLNAME="CTYS VirtualBox Match-Filter"
#
#CALLFULLNAME:
CALLFULLNAME="vboxmatch.sh"
#
#LICENCE:
LICENCE=GPL3
#
...
if [ -d "${HOME}/.ctys" -a -d "${HOME}/.ctys/vbox" ];then
	...
	if [ -f "${MYLIBEXECPATH}/../conf/ctys/vbox/vbox.conf-${MYOS}.sh" ];then
	    ...
	    ABORT=1;
	    printERR $LINENO $BASH_SOURCE ${ABORT} "Missing system tools configuration file:\"vbox/vbox.conf-${MYOS}.sh\""
	    printERR $LINENO $BASH_SOURCE ${ABORT} "Check your installation."
	    gotoHell ${ABORT}
	fi
    fi
...
#
#check ctys-conf-file-presence: ${Fname}.ctys
if [ -f "${FName}.ctys" ];then
    awk -F'=' -v matchMin=5 -f ${_myPKGBASE_VBOX_ENUMERATE}/enumfilter.awk {} \;
    exit $?
fi
...
if [  -f "${1%.*}.vdi" ];then
	...
	X=$(${VBOXMGR} showhdinfo $1|\
        awk -F':' '
           $1~/^UUID/{u=$2;}
           $1~/^Accessible/{a=$2;}
           END{gsub(" ","",a);gsub(" ","",u);print a";"u;a="";u="";}
           '
	    )

	printDBG $S_VMW ${D_UID} $LINENO $BASH_SOURCE "$FUNCNAME:X=$X"
	if [ "${X%;*}" == yes ];then
	    if [ -n "${X#*;}" ];then
		exit 0
	    fi
	...
	#this case is almost the same as missing access permissions.
	#for now decided to be a valid match, just with VMSTATE=DISABLED
	exit 0
    fi
fi
...
#here even any configuration file is missing.
#anyhow should not be reached due to find-params.
exit 1
