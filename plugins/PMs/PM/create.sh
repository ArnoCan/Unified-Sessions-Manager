...
_myPKGVERS_PM_CREATE="01.07.001b06"
...
function createConnectPM () {
	...
	function getMACFromCache () {
		...
		if [ -n "${_pname}" ];then
		    _myMAC=`${_VHOST} E:28:1 "${_pname}"`
		else
		    if [ -n "${_uuid}" ];then
			_myMAC=`${_VHOST} E:28:1 "${_uuid}"`
		    else
			if [ -n "${_label}" ];then
			    _myMAC=`${_VHOST} E:28:1 "${_label}"`
			else
			    if [ -n "${_fname}" ];then
				_myMAC=`${_VHOST} E:28:1 "${_fname}"`
			    else
				ABORT=1;
	...
	function getTCPFromCache () {
	    if [ -n "${_mac}" ];then
		_myIP=`${_VHOST} E:28:1 "${_mac}"`
	    else
		if [ -n "${_pname}" ];then
		    _myIP=`${_VHOST} E:28:1 "${_pname}"`
		else
		    if [ -n "${_uuid}" ];then
			_myIP=`${_VHOST} E:28:1 "${_uuid}"`
		    else
			if [ -n "${_label}" ];then
			    _myIP=`${_VHOST} E:28:1 "${_label}"`
			else
			    if [ -n "${_fname}" ];then
				_myIP=`${_VHOST} E:28:1 "${_fname}"`
			    else
				ABORT=1;
	...
	local _VHOST="${MYLIBEXECPATH}/ctys-vhost.sh ${C_DARGS} -C MACMAP -o TCP -p ${DBPATHLST} -s "
	if [ -n "${_tcp}" ];then
	    _myIP=$_tcp
	else
	    case ${C_NSCACHELOCATE} in
	...
	CHECKPARAM)
		...
		if((_unambig==0));then
		    ...
		    printERR $LINENO $BASH_SOURCE ${ABORT} "  TCP|MAC|UUID|LABEL"
 		    gotoHell ${ABORT}
		fi
	...
	EXECUTE)
		...
		if [ -n "${_woldelay}" ];then
   		...
   		printFINALCALL $LINENO $BASH_SOURCE "WAIT-TIMER:DomU(${_label},CTYS_PING_ONE_MAXTRIAL_PM,CTYS_PING_ONE_WAIT_PM)" "netWaitForPing \"${_myVM}\" \"${_pingcntPM}\" \"${_pingsleepPM}\""
	    ...
	    if [ "${_sshpingPM}" == 1 ];then
   		printFINALCALL $LINENO $BASH_SOURCE "WAIT-TIMER:DomU(${_label},CTYS_SSHPING_ONE_MAXTRIAL_PM,CTYS_SSHPING_ONE_WAIT_PM)" "netWaitForSSH \"${_myVM}\" \"${_sshpingcntPM}\" \"${_sshpingsleepPM}\" \"${_actionuserPM}\""
		...
		case ${_consoleType} in
			...
			_args="${_args} -t VNC -a create=l:CONSOLE,viewerdelay:5"
			if [ -n "$C_DARGS" ];then
			    _args="${MYLIBEXECPATH}/ctys.sh ${_args} ${_args1} ${_myPM}($C_DARGS)"
			else
			    _args="${MYLIBEXECPATH}/ctys.sh ${_args} ${_args1} ${_myPM}"
