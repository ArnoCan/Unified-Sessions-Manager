...
#
#DESCRIPTION:
#->Performs a complete setup of a basic VM on debian-5.0.0
#
#
...
#some local defaults
#
_rootdrv=MARKER_HDD_A
_swapdrv=MARKER_HDD_B
...
echo "******************************************************************"
echo
echo "Initial storage setup for PARA virtualized Xen-DomU on debian-5"
echo
echo
...
if [ "${_BALLOON}" == 'y' ];then
    ...
    if [ -e "${_rootdrv}" ];then
	echo "Already present, reuse it. If recreation remove manually."
    else

	_bsx=${_bc%%[^0-9]}
	 _unit=${_bc##[0-9]}; _unit=${_unit//[0-9]/}
	if((_bsx<1));then
	    echo "Blockcount for image creation is not valid:$_bsx">&2
	    exit 127;
	fi
    ...
    if [ -e "${_swapdrv}" ];then
	echo "Already present, reuse it. If recreation  required remove manually."
    else
...
echo "/dev/MARKER_HDD_a   /      ext3   defaults,errors=remount-ro  0 0">>${_debootanchor}/etc/fstab
echo "/dev/MARKER_HDD_b  swap    swap   sw  0 0">>${_debootanchor}/etc/fstab
