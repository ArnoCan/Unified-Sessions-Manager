...
#FUNCEND###############################################################
function getMEMinfo () {
    local _ram=`sysctl -n hw.realmem`
...
function getHDDinfo () {
    ...
    local _nlst=${*}
    if [ -z "${_nlst}" ];then
	local _nlst=$(sysctl -n kern.disks)
	for i in ${_nlst//,/ };do
	    if [ "${i//ad}" != "${i}" ];then
		if [ -n "$_dlst" ];then
		    _dlst="${_dlst}%${i}"
	...
	for i in $_nlst;do
	    _cur=$(df -lh ${i} 2>/dev/null|awk '/^\/dev/{gsub(".*dev\/","",$1);gsub("s[0-9][a-z]$","",$1);printf("%s",$1);}')
	    if [ -n "$_cur" ];then
		if [ -n "$_dlst" ];then
		    _dlst="${_dlst}%${_cur}"
		else
