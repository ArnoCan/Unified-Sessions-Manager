...
if [ -n "$_ping" ];then
	...
	${CTYS_PING} -c ${PCNT:-1} -w ${PTIME:-1} ${i} 2>&1 >/dev/null
	if [ $? -eq 0 ];then           
...
if [ -n "$_ssh" -o -n "$_pm" -o -n "$_vm" ];then
	...
	if [ $dossh -eq 1 -a $? -eq 0 -o -n "${_VHOST}" ];then           
		...
		if [ -n "$_pm" ];then
		    ret=1
		    printDBG $S_BIN ${D_UID} $LINENO $BASH_SOURCE "Check PM"
			...
			ret=$?
                        ...
                        #try nodename
			_M=`${MYLIBEXECPATH}/ctys-vhost.sh ${C_DARGS} -o V -M all '^'${i}';.\*PM' 2>/dev/null`
			if [ -z "$_M" ];then
                            #try IP address
			    IP=`${CTYS_HOST} $i|awk '{printf("%s",$NF);}'`
			    printDBG $S_BIN ${D_UID} $LINENO $BASH_SOURCE "$FUNCNAME:i=$i <-> IP=$IP"
			    _M=`${MYLIBEXECPATH}/ctys-vhost.sh ${C_DARGS} -o V -M all '^'${IP}';.\*PM' 2>/dev/null`
			    if [ -n "$_M" ];then
				ret=0
			    fi
			else
			...
			printDBG $S_BIN ${D_UID} $LINENO $BASH_SOURCE "$FUNCNAME:_M=$_M"
		    fi
    	            if [ "$ret" == "0" ];then           
			[ -z "$_reverse" ]&&TMPLST="${TMPLST} ${i}"
			((success++))
		...
		if [ "$ret" != "0" -a -n "$_vm" ];then
                    ret=1;
		    printDBG $S_BIN ${D_UID} $LINENO $BASH_SOURCE "Check VM"
		    if [ -z "$_vhost"  -a -z "${_VHOST}" ];then
			ssh ${CTYS_NETACCOUNT}@${i} ls /etc/ctys.d/vm.conf 2>&1 >/dev/null
			ret=$?
		    else
                        #try nodename
			_M=`${MYLIBEXECPATH}/ctys-vhost.sh ${C_DARGS} -o V -M all '.\*;'${i}';.\*VM' 2>/dev/null`
			if [ -z "$_M" ];then
                            #try IP address
			    IP=`${CTYS_HOST} $i|awk '{printf("%s",$NF);}'`
			    printDBG $S_BIN ${D_UID} $LINENO $BASH_SOURCE "$FUNCNAME:i=$i <-> IP=$IP"
			    _M=`${MYLIBEXECPATH}/ctys-vhost.sh ${C_DARGS} -o V -M all '.\*;'${IP}';.\*VM' 2>/dev/null`
			    if [ -n "$_M" ];then
				ret=0
			    fi
			else
