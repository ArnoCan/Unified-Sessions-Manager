...
[ -z "$CTYS_UNLINK" ]&&CTYS_UNLINK=`getPathName $LINENO $BASH_SOURCE ERROR unlink /usr/bin`
[ -z "$CTYS_SYSCTL" ]&&CTYS_SYSCTL=`getPathName $LINENO $BASH_SOURCE ERROR sysctl /sbin`
[ -z "$CTYS_LSPCI" ]&&CTYS_LSPCI=`getPathName $LINENO $BASH_SOURCE WARNING lspci /usr/bin`
...
[ -z "$CTYS_APTITUDE" ]&&CTYS_APTITUDE=`getPathName $LINENO $BASH_SOURCE WARNINGEXT aptitude /usr/bin`
