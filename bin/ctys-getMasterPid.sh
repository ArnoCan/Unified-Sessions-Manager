...
#
#FULLNAME:
FULLNAME="ctys-getMasterPid.sh"
#
#CALLFULLNAME:
CALLFULLNAME="CTYS gets master PID for given params"
#
#LICENCE:
LICENCE=GPL3
#
...
#path for various loads: libs, help, macros, plugins
MYHELPPATH=${MYHELPPATH:-$MYLIBPATH/$MYLANG}
...
#	'-')shift;_listmembers=${1};shift;;
	...
	'-h'|'--help'|'-help')showToolHelp;exit 0;;
        ...
        *)  #server list
	    ...
	    argLst="${argLst## }";
	    argLst="${argLst%% }";
	    printDBG $S_BIN ${D_UID} $LINENO $BASH_SOURCE "argLst=$argLst"
	    shift;
	    ;;
    esac
done

procFindTopBottom $argLst
